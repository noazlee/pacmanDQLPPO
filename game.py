    ...
    def info(self):
        ...
        try:
            info_dict = {
                ...
                "highscores": self.highscores if self.highscores else []
            }
            ...
            # If there's any error, return a minimal working response
            print(f"Error in game.info(): {e}")
            minimal_info = {
                "map": getattr(self.map, 'filename', 'unknown'),
                "ghosts": self._n_ghosts,
                "ghosts_level": self._l_ghosts,
                "fps": 10,
                "timeout": 3000,
                "lives": 3,
                "points": {"energy": 1, "boost": 10, "ghost": 50, "time_bonus": 1},
                "boost_timeout": 30,
                "highscores": []
            }
            return json.dumps(minimal_info)
