...
class Ghost:
    ...
    # Replace the entire update method in ghost2.py (around lines 200-240)
        ...
        if self.wait > 0:
            ...
            if 'pacman' in state:
                ...
                if distance(g_pos, self.map.ghost_spawn) <= self.respawn_dist and distance(p_pos, self.map.ghost_spawn) > self.respawn_dist:
                    ...
                    if not self.plan or len(self.plan) == 0:  # FIXED: Check for None
                        exit_plan = self.find_exit(g_pos, [], [])
                        self.plan = exit_plan if exit_plan is not None else []
                    
                    if self.plan and len(self.plan) > 0:
                        self.direction = self.plan.pop(0)
                    else:
                        # Fallback: move away from spawn
                    ...
                    mdepth = min(distance(g_pos, p_pos), 10)  # FIXED: Limit search depth
                    delta = 0
                    if mdepth >= 4:
                        delta = 3
                    
                    tracking_plan = self.find_path(g_pos, p_pos, lghosts, 0, mdepth, [], [], delta)
                    self.plan = tracking_plan if tracking_plan is not None else []
                    
                    logger.debug("Plan = %s", self.plan)
                    if self.plan and len(self.plan) > 0:
                        self.direction = self.plan.pop(0)
                    else:
                        # Fallback: move towards pacman using simple directions
                        dirs = self.directions(p_pos, g_pos)
                        for d in dirs:
                            npos = self.map.calc_pos(g_pos, d)
                            if npos != g_pos and npos not in lghosts:
                ...
                # FIXED: Safer buffer update
                try:
                    next_pos = self.map.calc_pos((self.x, self.y), self.direction)
                    self.buffer.add(next_pos)
                    # Update new position
                    self.x, self.y = next_pos
                except Exception as e:
                    logger.error(f"Error updating ghost position: {e}")
                    # Fallback: don't move
                    pass
